...
// Init Chain code initialization
func (t *BlockChainRealEstate) Init(stub shim.ChaincodeStubInterface) pb.Response {
	...
	var userNames = [6]string{"administrator", "① the owner", "② the owner", "③ the owner", "④ the owner", "⑤ the owner"}
	var balances = [6]float64{0, 5000000, 5000000, 5000000, 5000000, 5000000}
	//Initialized account data
...
// Invoke Implement the Invoke interface calling smart contract
func (t *BlockChainRealEstate) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	default:
		return shim.Error(fmt.Sprintf("No function: %s", funcName))
	}
}
