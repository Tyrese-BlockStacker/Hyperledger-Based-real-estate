...
// Test chain code initialization
...
func Test_QueryAccountList(t *testing.T) {
	stub := initTest(t)
	fmt.Println(fmt.Sprintf("1. Test all data\n%s",
...
func Test_CreateRealEstate(t *testing.T) {
	...
	//The operator should be an administrator and cannot be the same as everyone
	checkInvoke(t, stub, [][]byte{
		[]byte("createRealEstate"),
		[]byte("5feceb66ffc8"), //操作人
	...
	//Owner Proprietor information verification failed
	checkInvoke(t, stub, [][]byte{
		[]byte("createRealEstate"),
		[]byte("5feceb66ffc8"),    //操作人
...
func Test_CreateSelling(t *testing.T) {
	...
	//Verifying sales object Objectofsale belongs to the seller Seller's failure
	checkInvoke(t, stub, [][]byte{
		[]byte("createSelling"),
		[]byte(realEstateList[0].RealEstateID), //销售对象(正在出售的房地产RealEstateID)
